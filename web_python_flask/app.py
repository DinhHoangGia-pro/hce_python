...
# ---6. TEST TẤN CÔNG WEB SHELL------------
@app.route('/shell', methods=['GET', 'POST'])
def shell():
    output = ""
    cmd = ""

    # ✅ Allowlist lệnh (quan trọng)
    ALLOWED_COMMANDS = {
        "ls": ["ls", "-la"],
        "pwd": ["pwd"],
        "ssh": ["ssh", "dhgia@192.168.100.155", "ls -la"],
        "whoami": ["whoami"],
        "id": ["id"],
        "uname": ["uname", "-a"]
    }
    ...
    if request.method == 'POST':
        ...
        if cmd in ALLOWED_COMMANDS:
            try:
                # 🔥 CHẠY LỆNH THẬT (demo bản chất web shell)
    ...
    return render_template("shell.html", output=output, cmd=cmd)
# --- 7. GIỮ NGUYÊN TẤT CẢ CÁC TRANG TĨNH KHÁC CỦA ANH ---
